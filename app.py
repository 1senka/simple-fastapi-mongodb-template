...
async def create_product(new_product:Product ,user =  Depends(get_current_user)):
    product = MongoProduct(name=new_product.name,price=new_product.price,description=new_product.description)
    if(product): 
        product.save()
...
async def update_product(name:str,price:str,current_user: User = Depends(get_current_user)):
    ...
    if(product):
        if(price.isnumeric()):
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail="price is not numeric")
    else:
        raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail="product not found",
...
connect('my_db_fast_api', host='mongodb+srv://senka:SinaSenka@cluster0.aobxq.mongodb.net/yadakavaran?retryWrites=true&w=majority')
